...
Revision ID: 001
Revises: 
Create Date: 2026-03-11 20:57:00.000000
...
def upgrade() -> None:
    # Este script reflete o schema.sql inicial
    op.execute("""
...
def downgrade() -> None:
    op.execute("DROP TABLE xp_events")
    op.execute("DROP TABLE user_stats")
    op.execute("DROP TABLE review_attempts")
    op.execute("DROP TABLE review_sessions")
    op.execute("DROP TABLE learning_memory")
    op.execute("DROP TABLE lesson_attempt_details")
    op.execute("DROP TABLE user_lesson_state")
    op.execute("DROP TABLE user_progress")
